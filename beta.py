...
A1 = np.array([0.69, 0.66, 0.64, 0.66, 0.75, 0.78, 1.76, 4.49, 6.94, 6.37, 27.92]) #zählrate / zeit
M = np.array([1.30, 1.20, 1.08, 0.91, 0.82, 0.68, 0.54, 0.43, 0.41, 0.34, 0.27])#massenbelegung
A = np.log(A1)

#erste Gerade
A1g1 = np.array([1.76, 4.49, 6.94, 6.37, 27.92])#A1 zu erstw gerade
Mg1 = np.array([0.54, 0.43, 0.41, 0.34, 0.27])
Ag1 = np.log(A1g1)
...
#zweite Gerade
A1g2 = np.array([0.69, 0.66, 0.64, 0.66])
Mg2 = np.array([1.30, 1.20, 1.08, 0.91])
Ag2 = np.log(A1g2)
...
#Fehlerbalken
xfehler = np.array([0.003, 0.005, 0.003, 0.014, 0.003, 0.003, 0.003, 
            0.003, 0.001, 0.000, 0.000])
yfehler1 = np.array([0.03, 0.03, 0.03, 0.03, 0.03, 0.04, 0.06, 0.11, 
            0.15, 0.18, 0.53])
yfehler = np.log(yfehler1)
plt.errorbar(M, A, xerr=xfehler, yerr=yfehler, fmt='.', label="Messwerte")

xline1 = np.linspace(0.4, 1.3)
xline2 = np.linspace(0.27, 0.9)
#Graphen
...
plt.ylabel(r'$\ln ( A \cdot 1 \symup{s})$')
plt.legend()
plt.tight_layout()
#plt.xlim(0, 0.005)
#plt.ylim(0,0.0002)
